...
class RangeTrader(btcebot.TraderBase):
    ...
    def _attemptBuy(self, price, amount):
        ...
        if buy_amount >= btceapi.min_orders[self.pair]:
            logging.debug("attempting to buy %s %s at %s for %s %s" % (buy_amount, 
                curr1.upper(), price, buy_amount*price, curr2.upper()))
            if self.live_trades:
                r = self.api.trade(self.pair, "buy", price, buy_amount, conn)
                logging.debug("\tReceived %s %s" % (r.received, curr1.upper()))
    ...
    def onNewDepth(self, t, pair, asks, bids):
        ...
        self._log(ask_price, ask_amount)
...
def run(buy_floor, sell_ceiling, live_trades = False):        
...
if __name__ == '__main__':
    ...
    run(args.buy_floor, args.sell_ceiling) 
