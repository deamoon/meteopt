...
class RowParser(csvfeed.RowParser):
    ...
    def parseBar(self, csvRowDict):
        dateTime = datetime.datetime.strptime(csvRowDict["datetime"], "%Y-%m-%d %H:%M:%S.%f")
        price = float(csvRowDict["price"])
        volume = float(csvRowDict["volume"])
        return bar.BasicBar(dateTime, price, price, price, price, volume, None)
...
feed = Feed()
feed.addBarsFromCSV("TEST", "trades.csv")
strat = Strategy(feed)
strat.run()
